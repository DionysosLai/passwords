...
import passwords.shiqian.com.passwords.PasswordActivity;
...
public class PasswordAdapter extends RecyclerView.Adapter<PasswordAdapter.ViewHolder> {
    private List<PasswordsData> mProject = new ArrayList<>();
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private ViewHolder(final View view){
            super(view);
            projectView = view;
            passwordName = (TextView)view.findViewById(R.id.passwordcol_name);
        }
    }

    public PasswordAdapter(List<PasswordsData> projectItem, Activity activity){
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.passwordcol, parent, false);
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        if(Math.abs(event.getX() - moveX) < 5) {
                            ...
                            PasswordsData project = mProject.get(position);
                            Intent detailIntent = new Intent(mActivity, PasswordActivity.class);
                            detailIntent.putExtra("categoryid", project.getCategoryid());
                            detailIntent.putExtra("passwordid", project.getId());
                            detailIntent.putExtra("passactype", Const.pass_show);
    ...
    //  添加数据
    public void addData(int position, PasswordsData project) {
        if(mProject == null){
    ...
    public void addDatas(int position, List<PasswordsData> projects) {
        // 当数据为0，必须这样显示，否则无效
        if(getItemCount() <= 0) {
            position = 0;
    ...
    public void deleteData(int Position){
        ...
        LitePal.delete(PasswordsData.class, id);

        mProject.remove(Position);
        notifyItemRemoved(Position);
    }

    private void initDatas(PasswordsData project, int position, ViewHolder holder) {
        String passName = project.getName();
        long createTime = project.getCreatTime();
        Log.v("iniDatas", passName);
        holder.passwordName.setText(passName);
    }
...
//        layoutDialog.create().show();
    ...
    private void showEditDialog(final PasswordsData project, final View view) {
